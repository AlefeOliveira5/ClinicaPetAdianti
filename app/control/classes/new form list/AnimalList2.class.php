 ...
 * @author Alefe
 */
class AnimalList2 extends TPage
{
    protected $form;
    protected $datagrid;
    ...
    public function __construct()
    {
        ...
        $this->form = new BootstrapFormBuilder('list_animal');
        $this->form->setFormTitle('Animal');

        // 
        $opcao = new TCombo('opcao');
        $nome = new TEntry('nome_animal'); //variavel q vc vai buscar

        $items= array();
        $items['nome_animal'] = 'Nome Animal';
        $items['raca'] = 'Raça';

        $opcao->addItems($items);
        $opcao->setValue('nome_animal');
        ...
        $this->form->addAction('Novo',  new TAction(array('AnimalForm2', 'onEdit')), 'fa:plus green');
        ...
        // DATA GRID
        $nome_animal = new TDataGridColumn('nome_animal', 'Nome Animal', 'left');
        $raca = new TDataGridColumn('raca', 'Raça', 'left');

        $this->datagrid->addColumn($nome_animal);
        $this->datagrid->addColumn($raca);
      
        // fim data grid

        $actionEdit = new TDataGridAction(array('AnimalForm2', 'onEdit'));
    ...
    function Delete( $param = NULL )
    {
        try {
            ...
            $object = new Animal ($param['key']); // SEU RECORD <
    ...
    public function onReload( $param = NULL )
    {
        ...
        try {
            ...
            $repository = new TRepository('Animal');

            $criteria = new TCriteria;
            $criteria->setProperty('order', 'nome_animal');
